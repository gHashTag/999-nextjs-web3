...
const config: DocsThemeConfig = {
  ...
  head: () => {
    ...
    return (
      <>
        ...
        <meta property="og:url" content={`https://docs.huddle01.com${router.asPath}`} />
  ...
  sidebar: {
    ...
    titleComponent({ title, type: _type }) {
      const isSdkTitles = ['Javascript', 'ReactJS', 'React Native', 'Flutter', 'Server-SDK'].includes(title)
        ...
        return (
          <div className="flex items-center w-full justify-between">
            <div className="flex items-center gap-2.5">
              <span className="rounded-sm overflow-hidden">{SidebarIcons[title]}</span>
              {title}
            </div>
