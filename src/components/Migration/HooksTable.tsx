    ...
    <table className="w-full lg:mt-8 mt-4">
      ...
      <tbody className="p-8">
        {HooksData.map((item, i) => (
          <tr key={`hook-${item.old.one}`} className={`${i % 2 === 0 ? 'bg-[#1D1F22]/40' : 'bg-transparent'}`}>
            <td
              className={cn('border border-[#1D1F22] p-2', i < 6 && 'cursor-pointer underline')}
              role="presentation"
            ...
            <td className="border border-[#1D1F22] p-2 cursor-pointer">
              <span
                className={cn(
                  item.new.one && item.new.one === 'Deprecated' ? 'text-red-400 cursor-default' : null,
                  item.new.one?.startsWith('use') ? 'text-blue-500 underline' : null
                )}
                role="presentation"
              ...
              <span
                className={cn(item.new.two?.startsWith('use') ? 'text-blue-500 underline' : null)}
                role="presentation"
