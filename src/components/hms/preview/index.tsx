...
import { MicOffIcon, MicOnIcon, SettingsIcon, VideoOffIcon, VideoOnIcon, ArrowRightIcon } from '@100mslive/react-icons'
import {
  HMSPeer,
  useVideo,
  useHMSStore,
      ...
      <div className="w-[320px] flex flex-col md:ml-8 md:mt-0 mt-4 justify-between">
        ...
        >
          ...
          <p className="flex items-center">
            <InfoIcon className="mr-2" /> Note: Your mic is {audioEnabled ? 'on' : 'off'} and video is{' '}
            {videoEnabled ? 'on' : 'off'}
          </p>
...
const PreviewVideo: React.FC<{ videoTrack: HMSPeer['videoTrack'] }> = ({ videoTrack }) => {
  ...
  return <video className={`w-full h-full rounded-lg object-cover mirror`} autoPlay muted playsInline ref={ref} />
}
