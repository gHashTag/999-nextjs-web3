...
const VideoList = () => {
  ...
  return (
    <>
      <div className="w-full relative md:block hidden" style={{ height: 'calc((100vh - 3.2 * var(--header-height))' }}>
    ...
    <div ref={ref} className="w-full h-full flex flex-wrap place-content-center items-center">
      {pagesWithTiles &&
        ...
        pagesWithTiles[0].map((p, _) => (
          <VideoTile key={p.peer.id} width={p.width} height={p.height} trackId={p.peer.videoTrack || ''} />
        ))}
    </div>
  )
}

const ActiveSpeaker: React.FC<{ activePeer: HMSPeer; allPeers: HMSPeer[] }> = ({ allPeers, activePeer }) => {
  const isSomeoneScreenSharing = useHMSStore(selectIsSomeoneScreenSharing)
...
const AllSpeakers: React.FC<{ allPeers: HMSPeer[] }> = ({ allPeers }) => {
  ...
  return (
    ...
    >
      ...
      {pagesWithTiles.length > 1 ? <Pagination page={page} setPage={setPage} list={pagesWithTiles} /> : null}
    </div>
  )
}
