            ...
            <div key={m.id} className="flex items-start w-full mb-5 relative">
              ...
              <div className="flex flex-col flex-grow">
                <div className="w-full flex  items-center font-medium pl-2">
                  <span className="text-foreground">{m.sender === localPeer.id ? localPeer.name : m.senderName}</span>
                  {m.senderRole === 'stage' || m.senderRole === 'backstage' ? (
                    <Badge isLocal={m.sender === localPeer.id} isMod={m.senderRole === 'backstage'} />
                  ) : null}
              ...
              {localPeer.roleName === 'stage' || localPeer.roleName === 'backstage' ? (
                <div className="absolute top-0 right-0">
                  {localPeer.id !== m.sender ? <Dropdown role={m.senderRole || 'viewer'} id={m.sender} /> : null}
                </div>
              ) : null}
            </div>
          ))
